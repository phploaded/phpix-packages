...
$('.xtag-ctr').remove();
var spotdata = '<option value=""> --- Custom --- </option>';
for (let i=0; i<spotsArray.length; i++) {
spotdata = spotdata + '<option>'+spotsArray[i]+'</option>';
}

$(this).append('<div style="top:'+py+'%;left:'+px+'%;" class="xtag-ctr xtag-pos-'+xpos+'-'+ypos+'"><div class="xtag-inner">\
<div class="xtag-focus"></div>\
<div class="xtag-form">\
<select onchange="xtag_chooseSpot(this.value)" class="xtag-select">'+spotdata+'</select>\
...
if(confirm('Delete template tag?')){
var xval = $('.xtag-select').val();
$.get(gal_domain+"phpix-ajax.php?method=delete_template_tag&title="+xval, function(data) {
$('.xtag-select option').filter(function() {
return $(this).text() === xval; // Replace 'Option 2' with the text you want to match
}).remove();
$('.xtag-select').val('');
